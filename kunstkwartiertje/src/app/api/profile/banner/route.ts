...
export async function POST(request: Request) {
    ...
    try {
        ...
        const filePath = `banners/${user.id}/banner`;
        ...
        return NextResponse.json({ imageUrl: `${urlData.publicUrl}?v=${Date.now()}` });
    } catch (error) {
        console.error("Profile banner upload error", error);
        return NextResponse.json({ error: "Serverfout bij uploaden banner." }, { status: 500 });
    }
}
