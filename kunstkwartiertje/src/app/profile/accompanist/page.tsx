...
export default function AccompanistProfile() {
    ...
    return (
        ...
        <div className="w-full max-w-5xl lg:max-w-6xl mx-auto p-4 md:p-8">
            {/* Header Section */}
            <div className="profile-card rounded-none shadow-sm border border-gray-100 overflow-hidden mb-8">
                <div className="relative h-32 md:h-48 w-full overflow-hidden" style={{
                ...
                <div className="px-6 pb-6 relative">
                    <div className="flex flex-col md:flex-row items-center md:items-end gap-6 -mt-16 md:-mt-20 mb-4">
                        ...
                        <div className="mt-4 grid w-full grid-cols-1 gap-2 sm:grid-cols-3 md:mt-0 md:w-auto">
                            ...
                            <button type="button" onClick={openEditProfile} className="px-4 py-2 bg-gray-100 text-gray-700 font-medium rounded-none hover:bg-gray-200 transition">
                                Edit Profile
                            </button>
                        </div>
            ...
            {(profileError || profileMessage) && (
                <p className={`mb-4 rounded-none px-3 py-2 text-sm ${profileError ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}>
                    {profileError ?? profileMessage}
                </p>
            )}

            <div id="artists-rights" className="mb-10 rounded-none border border-gray-200/70 bg-white p-5 shadow-sm">
                ...
                {(managementError || managementMessage) && (
                    <p
                        className={`mt-4 rounded-none px-3 py-2 text-sm ${
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-4">
                    <input
                        ...
                        className="rounded-none border border-zinc-200 px-3 py-2 text-sm text-zinc-800"
                    />
                    <input
                        type="email"
                ...
                <div className="mt-5 space-y-3">
                    {isLoadingManagedArtists ? (
                        <div className="rounded-none bg-gray-50 px-4 py-3 text-sm text-gray-700 ring-1 ring-gray-200">
                            Artiesten laden...
                        </div>
                    ) : managedArtists.length === 0 ? (
                        <div className="rounded-none bg-zinc-50 px-4 py-3 text-sm text-zinc-600 ring-1 ring-zinc-200">
                        ...
                        managedArtists.map((artist) => (
                            <div
                                key={artist.id}
                                className="rounded-none border border-zinc-200 bg-zinc-50/70 px-4 py-3"
                            >
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <span className="rounded-none bg-white px-2.5 py-1 text-[11px] font-semibold text-zinc-700 ring-1 ring-zinc-200">
                                    ...
                                    {[
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex items-center justify-between rounded-none border px-3 py-2 text-sm transition ${
                                                    enabled
                                ...
                                <div className="mt-3">
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="mt-1 w-full rounded-none border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-800"
                                    ...
                                    <div className="mt-2 flex justify-end">
                                        <button
                                            ...
                                            className="rounded-none bg-black px-4 py-1.5 text-xs font-semibold text-white hover:bg-gray-900 disabled:opacity-60"
                                        >
            ...
            <div className="mb-12 rounded-none border border-amber-200/80 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-1">
                ...
                {isLoadingManagedArtists ? (
                    <div className="mt-4 rounded-none border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700">
                        Inkomende wijzigingen laden...
                    </div>
                ) : pendingProfileArtists.length === 0 ? (
                    <div className="mt-4 rounded-none border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-600">
                    ...
                    <div className="mt-4 grid gap-4 lg:grid-cols-2">
                        {pendingProfileArtists.map((artist) => {
                            ...
                            return (
                                <div
                                    key={artist.id}
                                    className="rounded-none border border-amber-200 bg-amber-50/40 p-4 shadow-sm"
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex items-center gap-3">
                                            <img
                                                className="h-12 w-12 rounded-none object-cover ring-2 ring-white"
                                                src={
                                            ...
                                            <div>
                                                <Link
                                                    ...
                                                    className="text-sm font-semibold text-zinc-900 hover:text-gray-700"
                                                >
                                                    {artist.username || artist.email}
                                                </Link>
                                                <p className="text-xs text-zinc-500">{artist.email}</p>
                                        ...
                                        <span className="rounded-none bg-amber-100 px-2.5 py-1 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-200">
                                    ...
                                    <div className="mt-4 space-y-3 text-sm text-zinc-700">
                                        ...
                                        {typeof pending.proposedUsername === "string" && pending.proposedUsername !== artist.username && (
                                            <div className="rounded-none bg-white px-3 py-2 ring-1 ring-amber-100">
                                        ...
                                        {typeof pending.proposedProfilePic === "string" && pending.proposedProfilePic !== artist.profilePic && (
                                            <div className="rounded-none bg-white px-3 py-2 ring-1 ring-amber-100">
                                                ...
                                                <div className="mt-2 flex items-center gap-3">
                                                    <img
                                                        className="h-14 w-14 rounded-none object-cover ring-1 ring-zinc-200"
                                                    ...
                                                    <img
                                                        className="h-14 w-14 rounded-none object-cover ring-1 ring-amber-200"
                                    ...
                                    <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className="rounded-none border border-rose-200 bg-white px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-60"
                                            >
            ...
            <div>
                ...
                {isLoadingManagedArtists ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {managedArtists.map((artist) => (
                            <div key={artist.id} className="bg-white p-5 rounded-none border border-gray-100 shadow-sm hover:shadow-md transition relative flex flex-col items-center text-center group">
                                <Link href={`/profile/accompanist/artists/${artist.id}`} className="w-20 h-20 rounded-none overflow-hidden mb-4 border-2 border-transparent group-hover:border-blue-500 transition cursor-pointer mt-1 block">
                                    <img
                                        className="w-full h-full object-cover"
                                        src={
                                            artist.profilePic ||
                                ...
                                {artist.hasPendingProfileChanges && (
                                    <span className="mt-2 inline-flex rounded-none bg-amber-100 px-2.5 py-1 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-200">
                ...
                {isLoadingManagedArtworks ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {pendingArtworks.map((artwork) => (
                            <div key={artwork.id} className="bg-white rounded-none border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                                ...
                                <div className="p-5 flex-grow flex flex-col">
                                    ...
                                    <p className="text-sm text-gray-700 font-medium mb-3">Door: {artwork.artistName}</p>
                                    <p className="text-sm text-gray-600 mb-4 line-clamp-2">{artwork.description || "Geen beschrijving"}</p>
                                    <div className="mt-auto">
                                        <span className="inline-flex rounded-none bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">
                        ...
                        {approvedArtworks.map((artwork) => (
                            <div key={artwork.id} className="bg-white rounded-none border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                                ...
                                <div className="p-5 flex-grow flex flex-col">
                                    ...
                                    <p className="text-sm text-gray-600 mb-4 line-clamp-2">{artwork.description || "Geen beschrijving"}</p>
                                    <div className="mt-auto">
                                        <span className="inline-flex rounded-none bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">
                                            Gepubliceerd
                                        </span>
                                    </div>
                                </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={(event) => { if (event.target === event.currentTarget) setIsEditOpen(false); }}>
                <div className="w-full max-w-lg rounded-none bg-white p-6 shadow-2xl">
                    ...
                    <div className="mt-4 flex items-center gap-4">
                        <img src={editProfilePic || profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(editUsername || username)}&background=random`} alt="Profielfoto" className="h-16 w-16 rounded-none object-cover ring-1 ring-gray-200" />
                    ...
                    <label className="mt-4 flex flex-col gap-1.5 text-sm font-medium text-gray-700">
                        Gebruikersnaam
                        <input type="text" value={editUsername} onChange={(event) => setEditUsername(event.target.value)} className="rounded-none border border-gray-200 px-4 py-2" />
                    </label>

                    <label className="mt-4 flex flex-col gap-1.5 text-sm font-medium text-gray-700">
                        Bio
                        <textarea value={editAboutMe} onChange={(event) => setEditAboutMe(event.target.value)} rows={4} className="rounded-none border border-gray-200 px-4 py-2" />
                    </label>

                    <div className="mt-5 flex justify-end gap-2">
                        <button type="button" onClick={() => setIsEditOpen(false)} className="rounded-none bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200">Annuleren</button>
                        <button type="button" onClick={() => void handleSaveProfile()} disabled={isSavingProfile} className="rounded-none bg-black px-4 py-2 text-sm font-semibold text-white hover:bg-gray-900 disabled:opacity-60">{isSavingProfile ? "Opslaan..." : "Opslaan"}</button>
                    </div>
                </div>
            </div>
        )}
        ...
        {isUploadOpen && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4" onClick={(event) => { if (event.target === event.currentTarget) setIsUploadOpen(false); }}>
                <div className="w-full max-w-md rounded-none bg-white p-6 shadow-2xl">
                    ...
                    <input type="file" accept="image/*" onChange={(event) => setUploadFile(event.target.files?.[0] ?? null)} className="mt-4 w-full rounded-none border border-gray-200 px-3 py-2 text-sm" />
                    <div className="mt-5 flex justify-end gap-2">
                        <button type="button" onClick={() => setIsUploadOpen(false)} className="rounded-none bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200">Sluiten</button>
                        <button type="button" onClick={() => void handleUploadPicture()} disabled={isUploadingPicture || !uploadFile} className="rounded-none bg-black px-4 py-2 text-sm font-semibold text-white hover:bg-gray-900 disabled:opacity-60">{isUploadingPicture ? "Uploaden..." : "Upload"}</button>
                    </div>
                </div>
            </div>
        )}
